 ...
 */
class MP_Rest_Api_Controller extends WC_REST_Controller {
	...
	public function firebase_token_refresh_callback( WP_REST_Request $request ) {
		...
		$response_data = array(
			...
			'license_key'              => esc_attr( get_option( 'magni_pos_license_key' ) ),
			'item_id'                  => esc_attr( get_option( 'magni_pos_item_id' ) ),
	...
	public function forgot_password_callback( WP_REST_Request $request ) {
		...
		if ( empty( $username_req ) || ! is_string( $username_req ) ) {
			return $this->send_error( 'empty_username', esc_html__( 'Enter a username or email address.', 'magni-pos' ), 400 );
		} elseif ( strpos( $username_req, '@' ) ) {
			$user_data = get_user_by( 'email', trim( wp_unslash( $username_req ) ) );
			if ( empty( $user_data ) ) {
				return $this->send_error( 'invalid_email', esc_html__( 'There is no account with that username or email address.', 'magni-pos' ), 400 );
			}
		} else {
		...
		if ( $message && ! wp_mail( $user_email, wp_specialchars_decode( $title ), $message ) ) {
			return $this->send_error( 'retrieve_password_email_failure', esc_html__( 'The email could not be sent. Your site may not be correctly configured to send emails.', 'magni-pos' ), 401 );
		}
