 ...
 */
class MP_Customer_Api_Controller extends WC_REST_Controller {
	...
	public function customer_check_create_callback( WP_REST_Request $request ) {
		...
		if ( ! $user ) {
			...
			$wc_customer->set_email( sanitize_email( $contact . '@magnipos.com' ) );
			$wc_customer->set_billing_email( sanitize_email( $contact . '@magnipos.com' ) );
	...
	public function add_customer_balance( $user_id, $order_id, $due_date, $payment_method, $remark, $created_date, $created_date_gmt, $amount, $total, $status ) {
		...
		if ( $wpdb->insert_id ) {
			...
			do_action( 'magnipos_customer_billing_added', $data );
		}
