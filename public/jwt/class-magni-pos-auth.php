...
class Magni_Pos_Auth {
	...
	public function __construct() {
		$this->namespace = 'magni-pos/v1';
		$this->messages  = array(
			'jwt_auth_no_auth_header'  => esc_html__( 'Authorization header not found.', 'magni-pos' ),
			'jwt_auth_bad_auth_header' => esc_html__( 'Authorization header malformed.', 'magni-pos' ),
		);
	}
	...
	public function add_cors_support() {
		$enable_cors = defined( 'MAGNI_JWT_AUTH_CORS_ENABLE' ) ? MAGNI_JWT_AUTH_CORS_ENABLE : false;
	...
	public function authenticate_user( $username, $password, $custom_auth = '' ) {
		...
		if ( $custom_auth ) {
			$custom_auth_error = new WP_Error( 'jwt_auth_custom_auth_failed', esc_html__( 'Custom authentication failed.', 'magni-pos' ) );
	...
	public function generate_token( $user, $return_raw = true ) {
		...
		$response = array(
			...
			'message'    => __( 'Credential is valid', 'magni-pos' ),
			'data'       => array(
				'token'       => $token,
				'id'          => $user->ID,
	...
	public function validate_token( $return_response = true ) {
		...
		try {
			...
			if ( $payload->iss !== $this->get_iss() ) {
				...
				return new WP_REST_Response(
					array(
						...
						'message'    => esc_html__( 'The iss do not match with this server.', 'magni-pos' ),
						'data'       => array(),
					),
					403
				);
			...
			if ( ! isset( $payload->data->user->id ) ) {
				...
				return new WP_REST_Response(
					array(
						...
						'message'    => esc_html__( 'User ID not found in the token.', 'magni-pos' ),
						'data'       => array(),
					),
					403
				);
			...
			if ( ! $user ) {
				...
				return new WP_REST_Response(
					array(
						...
						'message'    => esc_html__( "User doesn't exist", 'magni-pos' ),
						'data'       => array(),
					),
					403
				);
			...
			if ( ! empty( $failed_msg ) ) {
				...
				return new WP_REST_Response(
					array(
						...
						'message'    => esc_html__( 'Token is obsolete', 'magni-pos' ),
						'data'       => array(),
					),
					403
				);
			...
			$response = array(
				...
				'message'    => esc_html__( 'Token is valid', 'magni-pos' ),
				'data'       => array(),
			);
	...
	public function determine_current_user( $user_id ) {
		...
		if ( $this->is_error_response( $payload ) ) {
			if ( 'jwt_auth_no_auth_header' === $payload->data['code'] ||
				...
				if ( $rest_api_slug . '/magni-pos/v1/token' !== $request_uri ) {
					...
					$is_ignored    = true;
					$magni_pos_url = $rest_api_slug . '/magni-pos/v1';
					if ( false !== stripos( $request_uri, $magni_pos_url ) ) {
						$is_ignored = false;
					}
