 ...
 */
class MP_Modify_Query_Controller {
...
WHERE
	...
	public function get_custom_wc_report_sale( $response, $sales_data, $request ) {
		...
		$customer_balance_table = $wpdb->prefix . 'mp_customer_balance';
	...
	public function custom_wc_rest_prepare_shop_order_object( $response ) {
		...
		if ( ! empty( $customer_gstin ) ) {
			$response->data['customer_gstin'] = get_user_meta( $customer_id, 'gstin_number', true );
		}
		...
		foreach ( $response->data['line_items'] as $item ) {
			...
			if ( ! empty( $variation_id ) ) {
				...
				if ( is_wp_error( $res ) ) {
					$item['product_variation_data'] = null;
				} else {
					$item['product_variation_data'] = $res->get_data();
				}
			}
			...
			if ( is_wp_error( $res2 ) ) {
				...
				$item['product_data'] = $res2->get_data();
			}
			$line_items[] = $item;
		}
