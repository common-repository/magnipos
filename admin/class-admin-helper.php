 ...
 * @since      0.9.0
 * @package    MagniPos
 * @subpackage MagniPos\Admin
 */

namespace MagniPos\Admin;
...
class Admin_Helper {
	...
	public static function get_activate_url( $redirect_to = null ) {
		if ( empty( $redirect_to ) ) {
		    $redirect_to = self::add_query_arg_raw(
		        array(
		            'page'  => 'magnipos',
		            'nonce' => wp_create_nonce( 'magni_pos_register_product' ),
		        ),
		...
		return self::add_query_arg_raw( $args, 'https://www.magnipos.com/login' );
	}
	...
	public static function get_registration_params() {
		...
		add_option( 'magni_pos_license_key',  $license_key );
		add_option( 'magni_pos_item_id', absint( $item_id ) );
		...
		<p class="notice notice-warning notice-alt notice-connect-disabled">
					<?php
						printf(
							...
							esc_html__( 'MagniPOS cannot be connected because your site URL doesn\'t appear to be a valid URL. If the domain name contains special characters, please make sure to use the encoded version in the %1$s &amp; %2$s fields on the %3$s page.', 'magni-pos' ),
							'<strong>' . esc_html__( 'WordPress Address (URL)', 'magni-pos' ) . '</strong>',
							'<strong>' . esc_html__( 'Site Address (URL)', 'magni-pos' ) . '</strong>',
							'<a href="' . esc_url( admin_url( 'options-general.php' ) ) . '">' . esc_html__( 'WordPress General Settings', 'magni-pos' ) . '</a>'
						);
					?>
		</p>
			<?php
	...
	public static function get_license_data() {
			...
			$item_id     = absint( get_option( 'magni_pos_item_id' ) );
			$url         =  self::getPOSUrl();
			...
			$request_url = $url . '/?' . http_build_query( $url_params );
